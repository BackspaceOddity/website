...
export const jetbrainsPage: ClientPage = {
  ...
  blocks: [
...

    {
      ...
      intro:
        'Для каждой работы выберите <strong>один</strong> фактор, который лучше всего описывает момент, когда потребность становится срочной. Не все сразу — только самый точный.',
      exerciseId: 'entry-points',
      ui: ru,
      questions: [
        {
          id: 'budget',
          q: 'Решать, куда направить следующий бюджет',
          example: 'когда именно это становится срочным — защита бюджета, конец квартала?',
          singleSelect: true,
          options: ['Почему — есть триггер', 'Когда — конкретный момент', 'Где — ситуация или место', 'На фоне чего — давление или событие', 'С кем — аудитория или стейкхолдер', 'С чем — инструменты или данные', 'С каким настроем — ощущение в момент'],
        },
        {
          id: 'creative',
          q: 'Понять, сработал ли креатив',
          example: 'ролик висит, результат неизвестен — кто-то спрашивает?',
          singleSelect: true,
          options: ['Почему — есть триггер', 'Когда — конкретный момент', 'Где — ситуация или место', 'На фоне чего — давление или событие', 'С кем — аудитория или стейкхолдер', 'С чем — инструменты или данные', 'С каким настроем — ощущение в момент'],
        },
        {
          id: 'value',
          q: 'Доказать ценность маркетинга руководству',
          example: 'приходится объяснять, что год маркетинга принёс?',
          singleSelect: true,
          options: ['Почему — есть триггер', 'Когда — конкретный момент', 'Где — ситуация или место', 'На фоне чего — давление или событие', 'С кем — аудитория или стейкхолдер', 'С чем — инструменты или данные', 'С каким настроем — ощущение в момент'],
        },
        {
          id: 'produced',
          q: 'Знать, что кампания на самом деле принесла',
          example: 'кампания завершилась, данные разрозненные — нужна картина?',
          singleSelect: true,
          options: ['Почему — есть триггер', 'Когда — конкретный момент', 'Где — ситуация или место', 'На фоне чего — давление или событие', 'С кем — аудитория или стейкхолдер', 'С чем — инструменты или данные', 'С каким настроем — ощущение в момент'],
        },
        {
          id: 'compare',
          q: 'Сравнивать кампании по одной мерке',
          example: 'нужно решить, что работает лучше, а метрики у всех разные?',
          singleSelect: true,
          options: ['Почему — есть триггер', 'Когда — конкретный момент', 'Где — ситуация или место', 'На фоне чего — давление или событие', 'С кем — аудитория или стейкхолдер', 'С чем — инструменты или данные', 'С каким настроем — ощущение в момент'],
        },
        {
          id: 'brief',
          q: 'Брифовать следующую кампанию умнее',
          example: 'начинается планирование, но база от прошлого не накоплена?',
          singleSelect: true,
          options: ['Почему — есть триггер', 'Когда — конкретный момент', 'Где — ситуация или место', 'На фоне чего — давление или событие', 'С кем — аудитория или стейкхолдер', 'С чем — инструменты или данные', 'С каким настроем — ощущение в момент'],
        },
      ],
