...
(function () {
  ...
  function placeMarker(it,rail){ var t=it.t,r=it.rect,m=mk('div'); m.setAttribute('data-em-marker','1'); m.dataset.id=it.id; m.title=(t.type==='visual'?'Visual':t.type==='tov'?'Tone of voice':'Copy')+' comment — click to open'; var sx=window.pageXOffset,sy=window.pageYOffset;
    ...
    else if(t.type==='visual'){ var top=r.top+sy-9,left=r.right+sx-9; m.style.cssText='position:absolute;'+base+'top:'+top+'px;left:'+left+'px;width:22px;height:22px;border-radius:50% 50% 50% 3px;background:var(--ink);color:var(--paper);border:2px solid var(--paper);'; m.innerHTML=BUB; }
