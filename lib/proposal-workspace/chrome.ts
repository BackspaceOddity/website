...
(function () {
  ...
  function wsRow(o){ var cw=twSaved[o.w]!==undefined?twSaved[o.w]:o.wd; var cs=twSaved[o.s]!==undefined?twSaved[o.s]:o.sd; return '<div style="display:flex;align-items:center;justify-content:space-between;padding:4px 6px;margin:0 -6px;border-bottom:1px solid var(--rule);"><span style="font-family:var(--mono);font-size:9px;letter-spacing:.05em;text-transform:uppercase;color:var(--ink-40);min-width:80px;">'+o.l+'</span><div style="display:flex;align-items:center;gap:5px;"><button class="ws-w" data-w="'+o.w+'" data-cur="'+cw+'" style="background:var(--surface);border:1px solid var(--rule-strong);border-radius:4px;padding:3px 8px;font-family:var(--mono);font-size:9px;text-transform:uppercase;color:var(--ink);cursor:pointer;min-width:56px;">'+WNAME[cw]+'</button><button class="ws-i" data-s="'+o.s+'" data-cur="'+cs+'" title="Italic" style="background:'+(cs==='italic'?'var(--ink)':'transparent')+';color:'+(cs==='italic'?'var(--paper)':'var(--ink-40)')+';border:1px solid var(--rule-strong);border-radius:4px;padding:3px 8px;font-family:var(--display);font-style:italic;font-size:11px;cursor:pointer;line-height:1;">I</button></div></div>'; }
  ...
  twPanel.querySelectorAll('.ws-w').forEach(function(b){ b.addEventListener('click',function(ev){ ev.stopPropagation(); var nxt=WCYCLE[Number(b.dataset.cur)]||400; b.dataset.cur=nxt; b.textContent=WNAME[nxt]; document.documentElement.style.setProperty(b.dataset.w,nxt); var s=twLoad(); s[b.dataset.w]=nxt; localStorage.setItem(TW_STORE,JSON.stringify(s)); }); });
  twPanel.querySelectorAll('.ws-i').forEach(function(b){ b.addEventListener('click',function(ev){ ev.stopPropagation(); var cur=b.dataset.cur==='italic'?'normal':'italic'; b.dataset.cur=cur; b.style.background=cur==='italic'?'var(--ink)':'transparent'; b.style.color=cur==='italic'?'var(--paper)':'var(--ink-40)'; document.documentElement.style.setProperty(b.dataset.s,cur); var s=twLoad(); s[b.dataset.s]=cur; localStorage.setItem(TW_STORE,JSON.stringify(s)); }); });
  updateStagedInd();
