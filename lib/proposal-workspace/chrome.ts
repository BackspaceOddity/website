...
(function () {
  ...
  var counter=mk('div'); counter.className='em-ui'; counter.style.cssText='position:fixed;top:46px;right:16px;z-index:10000;display:none;background:var(--ink);color:var(--paper);border-radius:10px;padding:2px 8px;font-family:var(--mono);font-size:9px;letter-spacing:.04em;text-transform:uppercase;pointer-events:none;'; document.body.appendChild(counter);
  ...
  function ours(el){ if(!el) return false; if(el.closest&&(el.closest('.em-ui')||el.closest('[data-em-marker]')||el.closest('.theme-toggle'))) return true; return [editBtn,counter,ring,hl,tip,dlg,card,(typeof twPanel!=='undefined'?twPanel:null),(typeof twBtn!=='undefined'?twBtn:null)].some(function(n){ return n&&n.contains&&n.contains(el); }); }
  ...
  /* ── markers ── */
  var markerEls={};
  function clearMarkers(){ for(var k in markerEls){ if(markerEls[k]&&markerEls[k].remove) markerEls[k].remove(); } markerEls={}; }
  function syncCounter(){ var n=Object.keys(load().threads).length; counter.textContent=n+' edit'+(n===1?'':'s'); counter.style.display=n?'block':'none'; }
  function elHl(el,on){ if(!on||!el){ hl.style.display='none'; return; } var r=el.getBoundingClientRect(); hl.style.top=r.top+'px'; hl.style.left=r.left+'px'; hl.style.width=r.width+'px'; hl.style.height=r.height+'px'; hl.style.display='block'; }
  ...
  function renderMarkers(){ clearMarkers(); var threads=load().threads; var rail=[]; var items=Object.keys(threads).map(function(id){ var el=findEl(threads[id]); return {id:id,t:threads[id],el:el,rect:el?el.getBoundingClientRect():null}; }); items.sort(function(a,b){ return (a.rect?a.rect.top:0)-(b.rect?b.rect.top:0); }); items.forEach(function(it){ if(!it.rect) return; placeMarker(it,rail); }); syncCounter(); }
  function placeMarker(it,rail){ var t=it.t,r=it.rect,m=mk('div'); m.setAttribute('data-em-marker','1'); m.dataset.id=it.id;
    if(t.type==='visual'){ var top=Math.max(4,r.top-10),left=Math.min(window.innerWidth-26,r.right-10); m.style.cssText='position:fixed;z-index:9996;top:'+top+'px;left:'+left+'px;width:20px;height:20px;border-radius:50%;background:var(--ink);color:var(--paper);border:2px solid var(--paper);box-shadow:0 1px 6px rgba(0,0,0,.30);cursor:pointer;display:flex;align-items:center;justify-content:center;font-family:var(--mono);font-size:10px;line-height:1;'; m.innerHTML='&#9998;'; }
    else { var y=r.top+2; rail.forEach(function(u){ if(Math.abs(y-u)<26) y=u+26; }); rail.push(y); y=Math.max(54,Math.min(window.innerHeight-30,y)); var glyph=t.type==='tov'?(t.tovStatus==='checking'?'&#8230;':(t.tovStatus==='error'?'&#33;':'&#10003;')):'&#9998;'; m.style.cssText='position:fixed;z-index:9996;top:'+y+'px;right:14px;min-width:22px;height:22px;padding:0 5px;border-radius:11px;background:'+(t.type==='tov'?'var(--surface)':'var(--paper)')+';color:var(--ink);border:1px solid var(--rule-strong);box-shadow:0 1px 5px rgba(0,0,0,.18);cursor:pointer;display:flex;align-items:center;justify-content:center;font-family:var(--mono);font-size:11px;line-height:1;'; m.innerHTML=glyph; }
    m.addEventListener('mouseenter',function(){ showTip(it); elHl(it.el,true); });
    m.addEventListener('mouseleave',function(){ hideTip(); elHl(null,false); });
