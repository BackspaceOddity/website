/**
 * Built-in page definitions as PURE DATA (BSO-684).
 *
 * The two real landings (8Figures sprint, brand transformation) expressed as
 * ordered `bt:` block-lists. Extracted from realpages.tsx so they can be imported
 * OUTSIDE the Next client bundle — by the builder (realpages.tsx) AND by the
 * one-time DB seed (scripts/seed-builtin-pages.ts, BSO-684).
 *
 * Deliberately self-contained: NO 'use client', NO component imports, NO module
 * imports at all. `content` is passed in as an argument, so Node can load this
 * file raw via --experimental-strip-types with zero module-resolution friction.
 * This is the single source for both renderers — content.ts is the seed input.
 */

type Block = { id: string; type: string; props: any; real: true };
const b = (id: string, type: string, props: any): Block => ({ id, type, props, real: true });

/* ---------- Brand transformation — content-independent literals ---------- */
export const PBT_PROJECTS = [
...
/* id -> { css: stylesheet id served by /builder-css/[id], blocks }.
   p8fig depends on `content` (the 8Figures doc); pbt is fully literal above. */
export function buildBuiltinPages(c: any) {
  const P8FIG = [
